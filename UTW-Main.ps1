...
Write-CrashLog "Boot: modules loaded at $($Script:BootClock.ElapsedMilliseconds) ms"
...
function Show-ProfileMatchDialog {
    <#
        "There is no megant on the new PC - is this the same person?"
...
function Show-MigrationGUI {
    ...
    try { $txtUSMTPath.Select(0, 0); $txtUSMTPath.ScrollToCaret() } catch { }
            ...
            $lblUSMTStatus.Text = "$($Script:CheckMark) USMT files found (scanstate, loadstate, migapp.xml, miguser.xml)"
            $lblUSMTStatus.ForeColor = $Script:T.Success
            $lblUSMTStatus.Tag = "status-ok"
...
$Script:NarrowPanels = @("Output", "Lookup")
    ...
    function Invoke-CatchUpSync {
        <#
            ...
            spelled the same on both machines - megant on the old PC, megans on
        ...
        # with, so a rename leaves C:\Users\megant behind an account called
        # megans.
    ...
    function Get-USMTPathForRun {
        ...
        if ($sel -match "64-bit") { return (Get-USMTPathForArch -BasePath $base -Arch "amd64") }
        if ($sel -match "32-bit") { return (Get-USMTPathForArch -BasePath $base -Arch "x86") }
        if (-not $TargetPC) { return $base }
...
"@
    ...
    # Which artwork the themes are drawn from. The XAML pictures are the ones
    # shipped in xaml\; turning this off falls every theme back to the GDI+
    # painters, which are kept precisely so there is something to fall back to.
    # "Hand-drawn artwork" was wrong twice over: nothing is hand-drawn, and the
    # two modes do not draw the same picture in the same places - XAML mode puts
    # ONE continuous backdrop behind the window, while the painters give every
    # surface its own. So it is a mode, not a style, and the name says only that.
    ...
    [void]$miHelp.DropDownItems.Add((New-MenuItem "UTW &notes (README)" {
        $p = Join-Path $Script:ScriptDir "UTW-README.md"
        if (Test-Path $p) { Start-Process $p }
        else { Show-ThemedMessage "UTW-README.md is not beside the script." "Notes" "OK" "Information" }
    }))
    ...
    if ($cache) {
        if ($cache.USMTPath -and (Test-USMTPath $cache.USMTPath)) {
            ...
            $lblUSMTStatus.Text = "$($Script:CheckMark) USMT files found"; $lblUSMTStatus.ForeColor = $Script:T.Success; $lblUSMTStatus.Tag = "status-ok"
