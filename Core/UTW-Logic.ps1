...
#  Config.xml policy
#
#  Config.xml turns off the three components that abort loadstate with error 72
#  on a NEWER-to-OLDER (or cross-build) migration. On a same-build migration it
#  does nothing useful and only changes what gets captured, so forcing it onto
#  every run - which an earlier build did - was a regression: same-build
#  captures that used to work started coming back with blank logs.
#
#  Mode:
#    auto (default) - apply only when the source and destination Windows builds
#                     are known to differ (set per-run by the build check)
#    yes            - always apply when Config.xml is present
#    no             - never apply
#
#  ConfigXmlBuildMismatch is reset at the start of every run and set true only
#  once two different build numbers have actually been read.
# ---------------------------------------------------------------------------
$Script:ConfigXmlMode          = "auto"
$Script:ConfigXmlBuildMismatch = $false

function Should-ApplyConfigXml {
    switch ("$($Script:ConfigXmlMode)".Trim().ToLower()) {
        "yes" { return $true }
        "no"  { return $false }
        default { return [bool]$Script:ConfigXmlBuildMismatch }
    }
}

function Get-RemoteOSBuild {
    <#
        Returns @{ Ok; Build (int); Display }. Ok=$false means it could not be
        asked - which is NOT the same as "same build", so the caller must treat
        an unknown build as "cannot compare" rather than "matches".
    #>
    param([Parameter(Mandatory)][string]$ComputerName)
    $pc = "$ComputerName".Trim()
    if (-not (Test-ValidComputerName $pc)) { return @{ Ok = $false; Build = 0; Display = "" } }
    try {
        ...
        $b = 0; [void][int]::TryParse("$($os.BuildNumber)", [ref]$b)
        return @{ Ok = ($b -gt 0); Build = $b; Display = "$($os.Caption) (build $($os.BuildNumber))".Trim() }
    } catch {
        return @{ Ok = $false; Build = 0; Display = "" }
    }
}

function Get-UsmtBuild {
    <#
        The Windows build the USMT binaries came from, taken from scanstate.exe's
        file version - e.g. 28000 from "10.0.28000.1". 0 if it cannot be read.
        Used to catch USMT from a pre-release / Insider ADK before it is run on
        released Windows, where it dies at the loader with 0xC0000139.
    #>
    param([Parameter(Mandatory)][string]$USMTPath)
    try {
        $exe = Join-Path $USMTPath "scanstate.exe"
        if (-not (Test-Path $exe)) { return 0 }
        $fv = "$((Get-Item $exe).VersionInfo.FileVersion)"
        if ($fv -match '\b\d+\.\d+\.(\d{4,7})\b') { return [int]$Matches[1] }
        return 0
    } catch { return 0 }
}

# Windows client/server builds that have shipped a released ADK carrying USMT.
# This only drives an early "does this look like a released ADK?" heads-up in
# the UI - the authoritative check is at run time, against the actual machine
# builds. ADD NEW RELEASED BUILDS HERE as ADKs ship (e.g. 26300 for 26H1).
$Script:KnownUsmtBuilds = @(
    10240, 10586, 14393, 15063, 16299, 17134, 17763, 18362, 18363,
    19041, 19042, 19043, 19044, 19045,     # Windows 10 -> 22H2
    20348, 25398,                           # Windows Server 2022 / 23H2
    22000, 22621, 22631,                    # Windows 11 21H2 -> 23H2
    26100, 26200                            # Windows 11 24H2 / 25H2
)

function Get-UsmtFolderInfo {
    <#
        Describes the USMT in a folder for the UI and the crash log.
        @{ Build (int, 0 = unknown); Released (bool); Summary (string) }

        Released is a heads-up, not a verdict. A build NEWER than every entry in
        KnownUsmtBuilds is the Insider/Canary-ADK signature - that USMT fails at
        the Windows loader (0xC0000139) on released machines.
    #>
    param([Parameter(Mandatory)][string]$USMTPath)
    $b = Get-UsmtBuild -USMTPath $USMTPath
    if ($b -le 0) { return @{ Build = 0; Released = $true; Summary = "version unreadable" } }
    if ($Script:KnownUsmtBuilds -contains $b) {
        return @{ Build = $b; Released = $true; Summary = "build $b - released ADK" }
    }
    $maxKnown = ($Script:KnownUsmtBuilds | Measure-Object -Maximum).Maximum
    if ($b -gt $maxKnown) {
        return @{ Build = $b; Released = $false
                  Summary = "build $b - NEWER than any released Windows ADK on record. If this is an Insider/Canary ADK it will fail on released machines (loader error 0xC0000139). Confirm it came from a released ADK." }
    }
    return @{ Build = $b; Released = $true; Summary = "build $b" }
}

function Write-UsmtVersionStamp {
    <#
        A note left in the USMT folder for whoever browses it later. PURELY
        informational - the tool never reads it back; every decision uses
        scanstate.exe's own file version. A read-only share just means no stamp.
    #>
    param([Parameter(Mandatory)][string]$USMTPath, [int]$Build = 0)
    if ($Build -le 0) { return }
    $f = Join-Path $USMTPath "USMT-VERSION.txt"
    try {
        if (Test-Path $f) {
            if ((Get-Content $f -Raw -ErrorAction Stop) -match "build\s+$Build\b") { return }
        }
        $txt = @(
            "USMT build $Build   (read from scanstate.exe file version)"
            "First recorded by UTW: $(Get-Date -Format 'yyyy-MM-dd')"
            ""
            "Use USMT from a RELEASED Windows ADK. Insider / Canary ADK USMT"
            "fails on released Windows with loader error 0xC0000139. A released"
            "ADK's USMT also captures from older Windows - build 26100 (Windows"
            "11 24H2) covers Windows 10 and Windows 11."
        ) -join "`r`n"
        Set-Content -LiteralPath $f -Value $txt -Encoding ASCII -ErrorAction Stop
    } catch { }
}
...
function Build-USMTCommand {
    ...
    # Config.xml normally lives in the USMT folder. Fall back to the tool's own
    # folders if it is not there. Only applied when the Config.xml policy says so
    # (see Should-ApplyConfigXml) - forcing it onto every run is what broke
    # same-build captures.
    $configXml = ""
    if (Should-ApplyConfigXml) {
        $configXml = Join-Path $USMTPath "Config.xml"
        if (-not (Test-Path $configXml)) {
            $alt = if (Get-Command Get-UTWResource -ErrorAction SilentlyContinue) { Get-UTWResource "Config.xml" }
                   elseif ($Script:ScriptDir) { Join-Path $Script:ScriptDir "Config.xml" }
            if ($alt -and (Test-Path $alt)) { $configXml = $alt }
        }
        if (Test-Path $configXml) { Write-Log "Config.xml found and applied: $configXml" } else { $configXml = "" }
    } else {
        Write-Log "Config.xml not applied (policy: $($Script:ConfigXmlMode))"
    }
        ...
        default {
            ...
            if ($isExport) {
                ...
                $a = Build-USMTArgs -Operation "Export" -StorePath $Ctx.ExportStore `
                        ...
                        -ConfigXml $(if ($Ctx.ApplyConfig) { "$xmlDir\Config.xml" } else { "" }) `
            ...
            if (($isExport -or $isImport) -and -not $Ctx.ApplyConfig -and "$($Script:ConfigXmlMode)".Trim().ToLower() -eq "auto") {
                $rows += New-Row "Config.xml" "(UTW)" `
                         "Not applied here. The 'auto' policy adds /config only when the source and destination Windows builds differ, which is checked when the run starts. Set it to 'yes' in Settings to always apply it." `
                         "this PC" $false
            }
...
$Script:UTWDocsPath = "README.md"

function Get-UTWDocsRef {
    <#
        Where to send someone for a UTW-level failure. Returns the real path to
        the README if it can be found (beside the scripts or in Docs\), or just
        its name if not.
    #>
    if (Get-Command Get-UTWResource -ErrorAction SilentlyContinue) {
        $p = Get-UTWResource $Script:UTWDocsPath
        if ($p) { return $p }
    } elseif ($Script:ScriptDir) {
        $p = Join-Path $Script:ScriptDir $Script:UTWDocsPath
        if (Test-Path $p) { return $p }
    }
    return "$($Script:UTWDocsPath) (in the Docs folder)"
}
...
# NTSTATUS codes the Windows loader returns when scanstate.exe / loadstate.exe
# cannot even start. These are NOT USMT exit codes - the process died before
# main(). Every one of them, in this tool's context, means the USMT binaries do
# not match the Windows build they were told to run on - nearly always because
# the amd64 folder holds USMT from a pre-release / Insider ADK.
$Script:LoaderExitCodes = @{
    -1073741511 = "0xC0000139 STATUS_ENTRYPOINT_NOT_FOUND - a DLL is missing an export the binary needs"
    -1073741515 = "0xC0000135 STATUS_DLL_NOT_FOUND - a required DLL is missing"
    -1073741701 = "0xC000007B STATUS_INVALID_IMAGE_FORMAT - a DLL is the wrong architecture or build"
    -1073741502 = "0xC0000142 STATUS_DLL_INIT_FAILED - a DLL failed to initialise"
}
...
function Format-USMTExitLines {
    ...
    if ($Script:LoaderExitCodes.ContainsKey($Code)) {
        $rows += @{ Text = "Exit $Code - $($Script:LoaderExitCodes[$Code])"; Kind = "error" }
        $rows += @{ Text = "   This is a Windows loader failure, NOT a USMT error code - scanstate/loadstate never started."; Kind = "warn" }
        $rows += @{ Text = "   The USMT binaries are the wrong version for that machine's Windows build. Almost always USMT copied from a pre-release / Insider Windows ADK, which will not run on released Windows."; Kind = "warn" }
        $rows += @{ Text = "   Fix: replace the amd64 folder with USMT from a released ADK. Build 26100 (Windows 11 24H2) runs across this fleet. Keep the tuned XML; swap only Microsoft's binaries."; Kind = "warn" }
        return $rows
    }
...
function New-FactorySettingsFile {
    ...
    $defaults = [ordered]@{
        ...
        # Cross-build fix: auto applies Config.xml only when the two machines
        # report different Windows builds. See section 9 of the docs.
        ConfigXmlMode   = "auto"
    }
    try {
...
function Load-SettingsCache {
    ...
    try {
        ...
        XamlArt         = if ($null -ne $json.XamlArt)        { [bool]$json.XamlArt }        else { $null }
            # auto / yes / no. Absent means auto. See Should-ApplyConfigXml.
            ConfigXmlMode   = if ($json.ConfigXmlMode)   { "$($json.ConfigXmlMode)" }   else { $null }
        }
    } catch {
...
function Get-USMTPathForVersion {
    <#
        Given an arch-resolved USMT folder and the Windows build of the machine
        the USMT command will RUN ON, return the best-fitting folder.

        Extra USMT versions can be laid out two ways, and both are picked up:

          * SIBLINGS of the arch folder, named "<same name>-<build>" -
            e.g. amd64-19041 or amd64-26100 next to amd64.
          * a folder PER VERSION with the arch folder inside it - e.g.
            "USMT\Build 26100\amd64" and "USMT\Build 28000\amd64". Any folder
            beside this one's parent whose name carries a 4-7 digit number and
            holds "<arch>\scanstate.exe" counts.

        Each candidate's real build is read from its OWN scanstate.exe, never
        from the folder name. The pick is the newest set whose build is not
        newer than the target (certain to load there), or failing that the
        oldest available. The run-time check still confirms before the command
        is issued, and this is also how a stray Insider build gets passed over
        for a released one.

        Falls back to the given folder when there is nothing to choose from or
        the target build is unknown.
    #>
    param(
        [Parameter(Mandatory)][string]$BasePath,
        [int]$TargetBuild = 0
    )
    if ($TargetBuild -le 0) { return $BasePath }
    if (-not (Test-Path (Join-Path $BasePath "scanstate.exe"))) { return $BasePath }
    $leaf   = Split-Path $BasePath -Leaf
    $parent = Split-Path $BasePath -Parent

    $cands = @{}   # lower-case path -> @{ Path; Build }
    $addCand = {
        param([string]$p)
        if ([string]::IsNullOrWhiteSpace($p)) { return }
        $k = $p.ToLower()
        if ($cands.ContainsKey($k)) { return }
        if (-not (Test-Path (Join-Path $p "scanstate.exe"))) { return }
        $b = Get-UsmtBuild -USMTPath $p
        if ($b -gt 0) { $cands[$k] = @{ Path = $p; Build = $b } }
    }
    & $addCand $BasePath

    # amd64-26100 next to amd64
    if ($parent -and (Test-Path $parent)) {
        $rx = "^$([regex]::Escape($leaf))[-_.](\d{4,7})$"
        foreach ($d in (Get-ChildItem -LiteralPath $parent -Directory -ErrorAction SilentlyContinue)) {
            if ($d.Name -match $rx) { & $addCand $d.FullName }
        }
    }
    # USMT\Build 26100\amd64 , USMT\26100\amd64 , ... alongside this parent
    $grand = if ($parent) { Split-Path $parent -Parent } else { $null }
    if ($grand -and (Test-Path $grand)) {
        foreach ($d in (Get-ChildItem -LiteralPath $grand -Directory -ErrorAction SilentlyContinue)) {
            if ($d.Name -match '\d{4,7}') { & $addCand (Join-Path $d.FullName $leaf) }
        }
    }

    if ($cands.Count -le 1) { return $BasePath }
    $list = @($cands.Values)
    $fit = @($list | Where-Object { $_.Build -le $TargetBuild } | Sort-Object { [int]$_.Build } -Descending)
    if ($fit.Count -gt 0) { return $fit[0].Path }
    return (@($list | Sort-Object { [int]$_.Build }))[0].Path
}
...
function Copy-USMTToRemote {
    ...
    # Outside the skip branch on purpose. A machine staged by an older build has
    # the tools but not the Config, and "tools already staged, skipping" would
    # then skip the one file that was missing. Without it, loadstate hits error
    # 72 (V2V arbitration).
    if (-not (Test-Path (Join-Path $unc "Config.xml"))) {
        $altCfg = if (Get-Command Get-UTWResource -ErrorAction SilentlyContinue) { Get-UTWResource "Config.xml" }
                  elseif ($Script:ScriptDir) { Join-Path $Script:ScriptDir "Config.xml" }
        foreach ($cand in @((Join-Path $LocalUSMTPath "Config.xml"), $altCfg)) {
...
function Get-UsmtRunBatchContent {
    <#
    .SYNOPSIS
        The body of RunScan.bat / RunLoad.bat.

        Three things it does that the bare "scanstate.exe ... > log" did not:

          * mkdir the log folder first. If it is missing, the redirect fails
            before the exe runs, cmd writes the error to a console that does not
            exist under a SYSTEM task, and you get a zero-byte log and no clue.

          * write a banner line BEFORE the exe. After this, an empty stdout log
            means the task never executed the batch at all (policy, permissions,
            AppLocker) - a banner with nothing after it means the exe launched
            and hung - a banner plus an exit line means it ran and exited N.

          * preserve the exe's exit code past the trailing echo, so the task's
            Last Result is still USMT's code and Method A can read it.

        The banner is fixed literal text on purpose - the full command line is
        already written to the crash log on the PowerShell side, so nothing here
        has to echo an arg string that could carry a stray & or > .
    #>
    param(
        [Parameter(Mandatory)][string]$ToolPath,     # local-on-remote path to scanstate.exe / loadstate.exe
        [Parameter(Mandatory)][string]$ArgString,
        [Parameter(Mandatory)][string]$StdoutLog,    # local-on-remote
        [string]$Tag = "RunUSMT"
    )
    ...
    $lines = @(
        ...
        "echo [$Tag] %DATE% %TIME% host=%COMPUTERNAME% user=%USERNAME% starting > `"$StdoutLog`""
        "`"$ToolPath`" $ArgString >> `"$StdoutLog`" 2>&1"
        'set RC=%ERRORLEVEL%'
        "echo [$Tag] %DATE% %TIME% exit code: %RC% >> `"$StdoutLog`""
        'endlocal & exit /b %RC%'
    )
...
function Write-RemoteBatchFile {
    ...
    $cfgArg = ""
    if (-not (Should-ApplyConfigXml)) {
        Write-CrashLog "Config.xml not added to scanstate args (policy: $($Script:ConfigXmlMode))"
    } elseif (Test-Path $cfgUNC -ErrorAction SilentlyContinue) {
    ...
    $batchContent = Get-UsmtRunBatchContent -ToolPath "$RemoteUSMTPath\scanstate.exe" `
        -ArgString $argStr -StdoutLog $RemoteStdoutLog -Tag "RunScan"
    ...
    Write-CrashLog "RunScan.bat command: `"$RemoteUSMTPath\scanstate.exe`" $argStr"
}
...
function Start-RemoteScanTask {
    ...
    # Started is the wall clock the "has USMT actually begun?" watchdog measures
    # from - a task that sits at HAS_NOT_RUN or never writes a detail log is the
    # blank-log failure, and without a start time nothing can time it out.
    return @{ PC = $SourcePC; TaskName = $taskName; Started = (Get-Date) }
}

function Get-RemoteTaskStatus {
    <#
    .SYNOPSIS
        Non-throwing poll of a scheduled task's state. Called every timer tick.
        Returns @{ Finished; ExitCode; StatusText; NotYetRun; SawRunning }.

        Three independent methods, first conclusive one wins:
          A) schtasks /query /v : read the Last Result column (needs /v) and the
             Status text, parsed by COLUMN POSITION so it works on non-English
             Windows. Maps the SCHED_S_* HRESULTs: RUNNING/QUEUED means still
             going, HAS_NOT_RUN means the action has not started, anything else
             is the real process exit code.
          B) Progress log: "Successful run" or "errorCode, N".
          C) Detail log: "Successful run" or "USMT error code N". Catches error 72
             (V2V arbitration), which exits during PHASE Applying without an
             errorCode line in the progress log.
    #>
    param(
        [string]$PC,
        [string]$TaskName,
    ...
    # ---- Method A: schtasks /query  (locale-independent: parsed by column position) ----
    #
    # /v is REQUIRED. Without it the CSV carries only TaskName / Next Run Time /
    # Status - there is NO "Last Result" column - so the old code read a missing
    # field, fell back to the literal 267011 every time, and could never see a
    # result. Any failure that exits before USMT writes a log (blocked exe,
    # bad path, killed by policy) then looked like "still running" forever.
    #
    # Column NAMES are localised; column ORDER is not:
    #   0 HostName  1 TaskName  2 Next Run Time  3 Status
    #   4 Logon Mode  5 Last Run Time  6 Last Result
    # SCHED_S_* task-state HRESULTs (decimal): 267008 READY  267009 RUNNING
    #   267011 HAS_NOT_RUN  267014 TERMINATED  267045 QUEUED
    try {
        $raw = & schtasks /query /s $PC /tn $TaskName /v /fo CSV 2>&1
        if ($LASTEXITCODE -eq 0 -and $raw) {
            $row = $null
            try { $row = @($raw | Where-Object { $_ -match '","' } | ConvertFrom-Csv) | Select-Object -First 1 } catch { $row = $null }
            if ($row) {
                $vals       = @($row.PSObject.Properties.Value)
                $statusText = if ($vals.Count -gt 3) { "$($vals[3])".Trim() } else { "" }
                $lastRaw    = if ($vals.Count -gt 6) { "$($vals[6])".Trim() } else { "" }
                $lastInt    = 0
                $haveLast   = [int]::TryParse($lastRaw, [ref]$lastInt)

                $isRunning = ($statusText -match '(?i)runn|queue') -or
                             ($haveLast -and ($lastInt -eq 267009 -or $lastInt -eq 267045))
                if ($isRunning) {
                    return @{ Finished = $false; ExitCode = 267009; StatusText = "Running"; NotYetRun = $false; SawRunning = $true }
                }
                if ((-not $haveLast) -or $lastInt -eq 267011) {
                    # Never completed a run yet - either genuinely about to start,
                    # or the action failed to start. The caller's watchdog tells
                    # the two apart on elapsed time.
                    return @{ Finished = $false; ExitCode = 267011; StatusText = "NotYetRun"; NotYetRun = $true; SawRunning = $false }
                }
                # A run has completed. 267014 = it was terminated/killed; any
                # other value is the real process exit code (0 = success).
                Write-CrashLog "Task '$TaskName' on $PC finished via Method A: lastResult=$lastInt status=$statusText"
                return @{ Finished = $true; ExitCode = $lastInt; StatusText = $statusText; NotYetRun = $false; SawRunning = $false }
            }
        }
    } catch { }
    ...
    # None of the three methods confirmed completion. Do NOT claim "not yet run"
    # here - a flaky schtasks query on a healthy run lands here too; the caller
    # decides on detail-log presence instead.
    return @{ Finished = $false; ExitCode = 267011; StatusText = "Running"; NotYetRun = $false; SawRunning = $false }
}
...
function Invoke-RemoteExport {
    ...
    param(
        ...
        [bool]$Overwrite,
        [bool]$SettingsOnly = $false,
        [string]$StorePathOverride = "",   # e.g. \\DESTPC\UTWStore$\jdoe - store never lands on the source
...
function Write-RemoteImportBatchFile {
    ...
    param(
        ...
        [string]$RemoteStorePath,   # local-on-dest store path, e.g. C:\USMT Profiles\jdoe
        [string]$Username,
        [bool]$AllProfiles,
        [int]$Verbosity,
        [string]$RemoteLogFile,
    ...
    $cfgArg = ""
    if (-not (Should-ApplyConfigXml)) {
        Write-CrashLog "Config.xml not added to loadstate args (policy: $($Script:ConfigXmlMode))"
    } elseif (Test-Path (Join-Path $tempUNC "Config.xml") -ErrorAction SilentlyContinue) {
        $cfgArg = "$RemoteUSMTPath\Config.xml"
    ...
    $batchContent = Get-UsmtRunBatchContent -ToolPath "$RemoteUSMTPath\loadstate.exe" `
        -ArgString $argStr -StdoutLog $RemoteStdoutLog -Tag "RunLoad"

    $batchUNC = Join-Path (Get-RemoteTempUNC $DestPC) "RunLoad.bat"
    [System.IO.File]::WriteAllText($batchUNC, $batchContent, [System.Text.Encoding]::ASCII)
    Write-CrashLog "Wrote RunLoad.bat -> $batchUNC"
    Write-CrashLog "RunLoad.bat command: `"$RemoteUSMTPath\loadstate.exe`" $argStr"
}
...
function Start-RemoteLoadTask {
    ...
    return @{ PC = $DestPC; TaskName = $taskName; Started = (Get-Date) }
}
...
function Invoke-RemoteImport {
    ...
    param(
        [string]$DestPC,
        [string]$LocalUSMTPath,
        [string]$RemoteStorePath,   # local path on dest PC, e.g. C:\USMT Profiles\jdoe
...
function Get-ProfileNameMatches {
    <#
        ...
        The case it exists for: alicew on the old PC, alicex on the new one. A
        rename, a second account, a different naming standard - it happens often enough
    ...
    foreach ($c in @($Candidates | Where-Object { $_ })) {
        ...
        elseif ($t.StartsWith($s) -or $s.StartsWith($t)) { $score = 80 }
            ...
            # Longest shared prefix, as a percentage of the longer name. alicew
            # and alicex share five of six characters and score 83; alicew and
            # carlos share none and score 0.
...
function Get-ProfileSyncPreview {
    ...
    param(
        ...
        # ("alicew.CONTOSO") lives somewhere the username cannot predict. The
